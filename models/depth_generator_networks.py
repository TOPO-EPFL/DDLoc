...
class _DownSampleBlock(nn.Module):
    def __init__(self, input_nc, output_nc, norm_layer=nn.BatchNorm2d, nonlinearity=nn.PReLU(), use_bias=False):
        ...
        model = [
            nn.Conv2d(input_nc, output_nc, kernel_size=3, stride=2, padding=1, bias=use_bias),
            norm_layer(output_nc),
            nonlinearity
        ]
...
class _OneConvBlock(nn.Module):
    def __init__(self, input_nc, output_nc, norm_layer=nn.BatchNorm2d, nonlinearity=nn.PReLU(), use_bias=False):
        super(_OneConvBlock, self).__init__()
...
class _OutputBlock_no_tanh(nn.Module):
    def __init__(self, input_nc, output_nc, kernel_size=3, use_bias=False):
        super(_OutputBlock_no_tanh, self).__init__()
...
class _UNet_coord_down_8_skip_layer(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, layers=4, norm='instance', activation='PReLU', drop_rate=0, add_noise=False, gpu_ids=[],
                 weight=0.1):
        super(_UNet_coord_down_8_skip_layer, self).__init__()
        # this net is used for coordinate regression with ouput downsampled by 8
        # Input size: (B,3,480,720)
        # Output size: (B,3,60,90)
        ...
        # downsample 2 times before U-net
        self.down_conv1 = _OneConvBlock(input_nc, int(ngf/8), norm_layer, nonlinearity, use_bias)
        self.down_conv2 = _DownSampleBlock(int(ngf/8), int(ngf/4), norm_layer, nonlinearity, use_bias)
        self.down_conv3 = _DownSampleBlock(int(ngf/4), int(ngf/2), norm_layer, nonlinearity, use_bias)

        # encoder part
        self.pool = nn.AvgPool2d(kernel_size=2, stride=2)
        self.conv1 = nn.Sequential(
            nn.ReflectionPad2d(3),
            nn.Conv2d(int(ngf/2), ngf, kernel_size=7, padding=0, bias=use_bias),
            norm_layer(ngf),
            nonlinearity
        )
        self.conv2 = _EncoderBlock(ngf, ngf*2, ngf*2, norm_layer, nonlinearity, use_bias)
        ...
        self.output4 = _OutputBlock_no_tanh(ngf*(4+4), output_nc, 3, use_bias)
        self.output3 = _OutputBlock_no_tanh(ngf*(2+2)+output_nc, output_nc, 3, use_bias)
        self.output2 = _OutputBlock_no_tanh(ngf*(1+1)+output_nc, output_nc, 3, use_bias)
        self.output1 = _OutputBlock_no_tanh(int(ngf/2)+output_nc, int(ngf/2), 7, use_bias)

        self.upsample = nn.Upsample(scale_factor=2, mode='nearest')

        # downsample 1 time after U-net 
        self.down_conv4 = _DownSampleBlock(int(ngf/2), int(ngf/2), norm_layer, nonlinearity, use_bias)
        self.output_final = _OutputBlock_no_tanh(ngf*(1+1)+output_nc+int(ngf/2), output_nc, 7, use_bias)

        #layers used after interpolation
        self.conv_Interp_In = _OneConvBlock(int(ngf/2), int(ngf/2), norm_layer, nonlinearity, use_bias)
        self.conv_Interp_Out = _OneConvBlock(int(ngf/2), int(ngf/2), norm_layer, nonlinearity, use_bias)
        self.conv_Interp_Out_down = _OneConvBlock(ngf*(1+1)+output_nc, ngf*(1+1)+output_nc, norm_layer, nonlinearity, use_bias)


    def forward(self, input):
        input_down_sample = self.down_conv3(self.down_conv2(self.down_conv1(input)))
        input_ToUnet = F.interpolate(input_down_sample,(128,192)) #TODO: here change the size for your own implementation, it should be divisible by 16
        input_ToUnet = self.conv_Interp_In.forward(input_ToUnet)
        conv1 = self.pool(self.conv1(input_ToUnet))
        conv2 = self.pool(self.conv2.forward(conv1))
        conv3 = self.pool(self.conv3.forward(conv2))
        center_in = self.pool(self.conv4.forward(conv3))
        ...
        output_interp = F.interpolate(output1,(120,180)) #TODO: here change the size for your own implementation, it should be 2*(H,W), (H,W) is your output size
        output_interp = self.conv_Interp_Out.forward(output_interp)

        output = self.down_conv4(output_interp)

        u_net_down = torch.cat([deconv3, conv1 * self.weight * 0.1, self.upsample(output3)], 1)
        u_net_down = F.interpolate(u_net_down,(60,90)) #TODO: here change the size for your own implementation
        u_net_down = self.conv_Interp_Out_down.forward(u_net_down)
        output = self.output_final(torch.cat([output, u_net_down], 1))

        return output

class _UNet_coord_down_8_skip_layer_ft(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, layers=4, norm='instance', activation='PReLU', drop_rate=0, add_noise=False, gpu_ids=[],
                 weight=0.1):
        super(_UNet_coord_down_8_skip_layer_ft, self).__init__()
        # this net is used for coordinate regression with ouput downsampled by 8 and adapted for resize from data augmentation
        # Input size: (B,3,480*a,720*a) a is the scale factor range from 2/3 to 3/2
        # Output size: (B,3,60*a,90*a) 
    ...
    def forward(self, input, img_h, img_w, coords_h, coords_w):
        input_down_sample = self.down_conv3(self.down_conv2(self.down_conv1(input)))
        input_ToUnet = F.interpolate(input_down_sample,(128,192)) #TODO: here change the size for your own implementation, it should be divisible by 16
        input_ToUnet = self.conv_Interp_In.forward(input_ToUnet)
        conv1 = self.pool(self.conv1(input_ToUnet))
        ...
        output_interp = F.interpolate(output1,(int(img_h/4),int(img_w/4)))
        output_interp = self.conv_Interp_Out.forward(output_interp)

        output = self.down_conv4(output_interp)

        u_net_down = torch.cat([deconv3, conv1 * self.weight * 0.1, self.upsample(output3)], 1)
        u_net_down = F.interpolate(u_net_down,(int(coords_h),int(coords_w)))
        u_net_down = self.conv_Interp_Out_down.forward(u_net_down)
        output = self.output_final(torch.cat([output, u_net_down], 1))

...
class train_inpainting_module_I(base_model):
	def __init__(self, args, dataloaders_xLabels_joint):
		...
		self.save_model_name = ['inpaintNet', 'netD']
		...
		if self.isTrain:
			...
			if self.start_epoch > 0:
				self._load_models(self.save_model_name, self.start_epoch, isTrain=True,model_path=self.save_dir)
		...
		for epoch in range(self.start_epoch,self.total_epoch_num):
			...
			for sample_dict in self.dataloaders_xLabels_joint:
				...
				with torch.set_grad_enabled(phase=='train'):
					r2s_img = self.styleTranslator(imageListReal)
					...
					inpainted_r2s = self.inpaintNet(mod_r2s_img)
					...
					sta_list.append([float(total_loss),float(reconst_loss),float(perceptual_loss),float(style_loss)])
			...
			if (epoch+1) % self.save_steps == 0:
				print('***********************************************************')
				self.save_models(self.save_model_name, mode=epoch+1)
				print('Model of Epoch {} is saved'.format(epoch+1))
				print('***********************************************************')

			sta_list = np.mean(sta_list,axis=0)
			print('\tEpoch {}, average total loss: {:.7f}, reconst_loss: {:.7f}, perceptual_loss: {:.7f}, style_loss: {:.7f}'.format(
				epoch+1,sta_list[0],sta_list[1],sta_list[2],sta_list[3]))
